...
<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" >
        ...
        <div>
            ...
            <x-text-input id="username" class="block mt-1 w-full  border-gray-400 bg-table-bg-color text-gray-400" type="text" name="username" :value="old('username')" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('username')" class="mt-2" />
        </div>

        <!-- Password -->
        <div class="mt-4">
            <x-input-label for="password" :value="__('Contarseña')" />
